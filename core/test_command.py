...
from core.model_manager import ModelManager, ROLE_LABELS

_HERE = Path(__file__).resolve().parent
_FONT_DIR = _HERE.parent / "pack" / "fonts"
...
async def _test_request(upstream: dict, model_id: str, timeout: float = 12.0) -> dict:
    ...
    protocol = ModelManager.normalize_upstream_protocol(upstream.get("protocol")) or ModelManager.protocol_from_path(upstream.get("messages_path"))
    messages_path = ModelManager.endpoint_path_for_protocol(protocol)
    is_responses = protocol == "responses"
    is_openai = protocol in ("completions", "responses")
    ...
    if is_responses:
        payload = {"model": model_id, "max_output_tokens": 10,
...
async def handle_test_command(message, cleaned: str, mm: ModelManager, bot: NapcatActionTransport):
    """处理 /test 指令，bot 为业务侧 NapCat action transport。"""
    import shlex
    try:
    ...
    if arg.lower() == "alls":
        ...
        return
    if arg.lower() == "roles":
        await _cmd_all_roles(mm, bot, message)
        return
    if arg.lower() in ("role", "r") and len(parts) >= 3:
        await _cmd_test_role(mm, parts[2], bot, message)
        return
...
def _cmd_list(mm: ModelManager) -> str:
    ...
    return f"渠道: {ch_text}\n模型: {mdl_text}\n输入 /test <编号或名称> 测试；/test role <角色> 测角色实际渠道；/test roles 测全部角色"
...
async def _test_channel_first_ok(mm: ModelManager, channel: dict) -> tuple[bool, str]:
    """按渠道策略顺序测试，返回 (首个可用模型详情, 或全部失败原因汇总)。"""
    errors = []
    for m in (channel.get('models') or []):
        ...
        up = mm._find_upstream(up_name)
        if not up:
            errors.append(f'{up_name}/{mid}: 上游不存在')
            continue
        r = await _test_request(up, mid)
        if r['success']:
            return True, f'{up_name}/{mid} {r["elapsed_ms"]}ms'
        errors.append(f'{up_name}/{mid}: {r["error"]}')
    return False, ('; '.join(errors) if errors else '(渠道无模型)')

async def _cmd_test_role(mm: ModelManager, role: str, bot, message):
    """测试单个角色回退链解析后的实际渠道。"""
    role = str(role or '').strip()
    if role not in ROLE_LABELS:
        bot.send_text(message.chat_type, message.chat_id,
                      f'未知角色: {role}，可用: {", ".join(ROLE_LABELS.keys())}')
        return
    ch_name = mm.get_role_channel_name(role)
    if not ch_name:
        bot.send_text(message.chat_type, message.chat_id,
                      f'{ROLE_LABELS[role]} ({role}) 未解析到渠道（未配置）。')
        return
    channel = mm._find_channel(ch_name)
    if not channel:
        bot.send_text(message.chat_type, message.chat_id,
                      f'{ROLE_LABELS[role]} ({role}) 解析到的渠道 {ch_name} 不存在。')
        return
    ok, detail = await _test_channel_first_ok(mm, channel)
    label = f'{ROLE_LABELS[role]} ({role}) -> {ch_name}'
    if ok:
        bot.send_text(message.chat_type, message.chat_id, f'[OK] {label}: {detail}')
    else:
        bot.send_text(message.chat_type, message.chat_id, f'[FAIL] {label}: {detail}')

async def _cmd_all_roles(mm: ModelManager, bot, message):
    """测试所有角色（含分级三渠道）回退链解析后的实际渠道。"""
    lines = []
    for role, label in ROLE_LABELS.items():
        ch_name = mm.get_role_channel_name(role)
        if not ch_name:
            lines.append(f'[X] {label} ({role}): 未解析到渠道')
            continue
        channel = mm._find_channel(ch_name)
        if not channel:
            lines.append(f'[X] {label} ({role}): 渠道 {ch_name} 不存在')
            continue
        ok, detail = await _test_channel_first_ok(mm, channel)
        if ok:
            lines.append(f'[OK] {label} ({role}) -> {ch_name}: {detail}')
        else:
            lines.append(f'[FAIL] {label} ({role}) -> {ch_name}: {detail}')
    _send_multi(bot, message, lines, title='Test Roles')
