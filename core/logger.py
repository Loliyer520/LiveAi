...
日志分类（用于 entry.category 字段）：
  agent  - 常驻 agent（create_agent / run_agent_loop / destroy_agent）
  task   - 后台任务（tasker、set_alarm、notify_master 等；持久化 kind=dev_agent 为 tasker 兼容值）
...
DEFAULT_LOG_FILE_PATH = Path('data/logs/bot_debug.jsonl')
...
class BotLogger:
    ...
    def __init__(self, max_entries: int = 10000, log_file_path: str | Path | None = None):
        self._max_entries = max(100, int(max_entries or 10000))
        self._buffer: deque[LogEntry] = deque(maxlen=self._max_entries)
        self._lock = threading.Lock()
        self._log_file_path = Path(log_file_path) if log_file_path else DEFAULT_LOG_FILE_PATH
    ...
    def stats(self) -> dict:
        ...
        return {
            ...
            'log_file_path': str(self._log_file_path),
        }
    ...
    def _append_file_entry(self, entry: LogEntry) -> None:
        try:
            self._log_file_path.parent.mkdir(parents=True, exist_ok=True)
            payload = entry.to_dict()
            payload['message'] = str(payload.get('message') or '')[:10000]
...
def get_log_file_path() -> str:
    return get_bot_logger().get_log_file_path()
