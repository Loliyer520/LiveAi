...
_FALLBACK_FONT_PATHS = [
    ...
    # 项目自带（可手动放入 pack/fonts/）
]

# 字形支持缓存：避免重复调用 getmask()
_char_support_cache: dict[tuple, bool] = {}
# .notdef 字形 bytes 缓存（key: font id），用于区分真实字形与豆腐块
_notdef_bytes_cache: dict[int, bytes] = {}

# 私用区字符 U+E0FF，任何标准字体均无此字形，用于取 .notdef 基准
_NOTDEF_PROBE = "\ue0ff"


def _get_notdef_bytes(font) -> bytes:
    """获取字体的 .notdef 字形 mask bytes（首次调用时缓存）。"""
    fid = id(font)
    if fid not in _notdef_bytes_cache:
        try:
            mask = font.getmask(_NOTDEF_PROBE)
            _notdef_bytes_cache[fid] = bytes(mask)
        except Exception:
            _notdef_bytes_cache[fid] = b""
    return _notdef_bytes_cache[fid]


def _font_has_char(font, char: str) -> bool:
    """检测字体是否包含某个字符的真实字形（结果缓存）。

    修复：不能只检查 mask.size[0] > 0，因为不支持的字符会渲染 .notdef
    豆腐块，同样返回非零尺寸。需对比字符 mask bytes 与 .notdef bytes，
    两者相同则判为无字形，触发回退到 CJK 等字体。
    """
    key = (id(font), char)
    ...
    try:
        ...
        if mask is None or mask.size[0] == 0:
            ok = False
        else:
            notdef = _get_notdef_bytes(font)
            char_bytes = bytes(mask)
            # 若 mask bytes 与 .notdef 完全一致，则该字符无真实字形
            ok = not (notdef and char_bytes == notdef)
    except Exception:
        ok = False
...
def _resolve_font_for_char(primary_font, fallback_fonts: list, char: str):
    """为单个字符挑选实际要使用的字体。"""
    if _font_has_char(primary_font, char):
        return primary_font
    for fb in fallback_fonts:
        if _font_has_char(fb, char):
            return fb
    return primary_font
...
def _measure_text_with_fallback(
    ...
    if not needs_fallback:
        return measure_draw.textlength(text, font=primary_font)

    width = 0.0
    for ch in text:
        width += measure_draw.textlength(
            ch,
            font=_resolve_font_for_char(primary_font, fallback_fonts, ch),
        )
    return width
...
def _draw_text_with_fallback(
    ...
    md = measure_draw or draw
    for ch in text:
        resolved_font = _resolve_font_for_char(primary_font, fallback_fonts, ch)
        draw.text((x, y), ch, font=resolved_font, fill=fill)
        x += md.textlength(ch, font=resolved_font)
    return x
...
def render_code_to_image(
    ...
    *,
    ...
    # 计算最长行像素宽度。这里必须按真实 fallback 字体测量，否则中文会被低估。
    max_line_px = 0
    for segs in lines:
        text = "".join(s[0] for s in segs)
        max_line_px = max(
            max_line_px,
            int(_measure_text_with_fallback(text, font, fallback_fonts, tmp_draw)),
        )
    ...
    # 标题文字（居中）
    if title:
        tw = _measure_text_with_fallback(title, font, fallback_fonts, draw)
